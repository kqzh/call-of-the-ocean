...
function outFade77()     //淡出
{
    ctx.clearRect(0,0,860,540);
    ctx.drawImage(DieForBirthImage,0,0,canvas.width,canvas.height);
    ctx.drawImage(mask,0,0);
    maskCtx.globalAlpha -=0.015;
    maskCtx.clearRect(0,0,860,540);
    maskCtx.fillRect(0,0,860,540);
    if(maskCtx.globalAlpha>=0.03)
        requestNextAnimationFrame(outFade77);
    else
        MainDieForBirth();
}


function outFade66()     //淡入黑色
{
    ...
    if(maskCtx.globalAlpha<=0.9)
        requestNextAnimationFrame(outFade66);
    else
        ReadyToDieBirth();
}

function DieForBirth()
{
